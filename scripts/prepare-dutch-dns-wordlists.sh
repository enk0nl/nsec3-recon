...
if [[ ! -f "$src" ]]; then echo "missing expected Dutch DNS wordlist: $src"; echo "candidates:"; find "$DEPS" -type f \( -iname '*sub*domain*' -o -iname '*occurrance*' -o -iname '*occurrence*' \) 2>/dev/null | head -20; exit 1; fi
mkdir -p "$(dirname "$dst")"

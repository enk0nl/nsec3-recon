...
GIT_DEPS = {
    ...
    'nsec3-candidate-scheduler': ('https://github.com/enk0nl/nsec3-candidate-scheduler', 'SCHEDULER_REF', 'd47362b2342d007a13fff5d8301ccf7e9bd3c52f', 'deps/src/nsec3-candidate-scheduler'),
...
def write_dependency_manifest(ctx):
    ...
    for name, (remote, env_name, requested, rel) in GIT_DEPS.items():
        ...
        deps.append({'name': name, 'type': 'git', 'remote_url': remote, 'requested_ref': requested, 'resolved_commit': resolved, 'local_path': str(path)})
